...
`--buffer-depth=3`|Enable the paced output buffer with the specified frame capacity. Once primed it adds roughly `BufferDepth / fps` seconds of intentional latency. Set to `0` (default) to run zero-copy.
`--enable-output-buffer`|Shortcut to turn on paced buffering with the default depth of 3 frames (≈3 / fps seconds of latency).
