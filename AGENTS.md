...
* Wires Chromium audio through `CustomAudioHandler`, copying each planar float channel into a preallocated buffer that is passed to `NDIlib.send_send_audio_v2` (note: the current copy routine stores channel blocks sequentially while flagging them as interleaved).
...
[CustomAudioHandler]
  ...
  ├─ Copies planar float channels from CefSharp into sequential channel blocks (flagged for NDI as if interleaved)
...
* `CustomAudioHandler.cs` — Class `CustomAudioHandler`: implements `IAudioHandler`, allocates native float buffers per channel layout, copies each planar channel into contiguous blocks (still flagged as interleaved), and sends audio frames to NDI.
...
* **Audio:** `CustomAudioHandler.OnAudioStreamPacket` copies planar floats into channel-sized slabs inside a reusable buffer, fills `NDIlib.audio_frame_v2_t` (marked as interleaved with `channel_stride_in_bytes = sizeof(float)*samples`), and calls `NDIlib.send_send_audio_v2`.
