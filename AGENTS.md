...
* Chromium lifecycle: `AsyncContext.Run` + `SingleThreadSynchronizationContext` keep CefSharp happy on one STA-like thread. `CefWrapper.InitializeWrapperAsync` waits for the first page load, unmutes audio (CEF starts muted), subscribes to the `Paint` event, and starts a `FramePump` that either runs on a fixed cadence or services paced invalidation requests from the video pipeline. The watchdog remains active to keep the UI thread alive even when pacing temporarily pauses invalidations.
* Video path: `ChromiumWebBrowser.Paint` forwards frames to the `NdiVideoPipeline`. In zero-copy mode the pipeline sends the GPU buffer directly; when buffering is enabled it copies into a pooled ring buffer, waits until `BufferDepth` frames are queued, then drains the backlog FIFO while repeating the latest frame on underruns so cadence stays constant. With paced invalidation enabled the pipeline also decides when Chromium should request another repaint based on backlog, latency error, and cadence alignment telemetry, logging pause/resume transitions for diagnostics. Frame-rate metadata is advertised using either the configured target cadence or the measured average.
...
| `--use-paced-invalidation` | `--use-paced-invalidation` | Lets the paced pipeline gate Chromium invalidations based on backlog, latency, and cadence telemetry so Chromium only repaints when the pipeline is ready for another frame. Launcher enables this automatically when buffering is active. |
| `--disable-paced-invalidation` | `--disable-paced-invalidation` | Forces the legacy fixed-timer invalidation path even if buffering is enabled or the launcher default opted into pacing. |
...
* A `FramePump` invalidates Chromium on the cadence derived from `--fps` (or `--windowless-frame-rate`). When paced invalidation is enabled it services explicit requests from the video pipeline, pausing or cancelling work items when backlog/latency thresholds hit while the watchdog recovers if paint events stall entirely.
