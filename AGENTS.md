...
| `--use-paced-invalidation` | `--use-paced-invalidation` | When buffering is enabled the video pipeline will request Chromium invalidations on-demand, gating work until backlog, latency, and cadence telemetry indicate the renderer is ready. |
| `--disable-paced-invalidation` | `--disable-paced-invalidation` | Explicit opt-out of the paced invalidation path (for testing or when the launcher enables it by default). |
...
* A `FramePump` invalidates Chromium on the cadence derived from `--fps` (or `--windowless-frame-rate`) and contains a watchdog to recover if paint events stall. With paced invalidation enabled the pump defers invalidate requests whenever the video pipeline pauses pacing because backlog or sustained latency exceeded thresholds, and it logs pause/resume transitions for diagnostics. Cancellation tokens guard both the async request and UI-thread execution paths so outstanding work items exit quietly once shutdown begins.
...
* With paced invalidation enabled the `NdiVideoPipeline` tracks consecutive positive latency ticks and only pauses Chromium invalidation after the pipeline has remained behind schedule for multiple iterations. Latency and backlog resets (including re-entering warmup) clear that counter so pacing can resume promptly once telemetry normalises.
