...
* Video path: `ChromiumWebBrowser.Paint` forwards frames to the `NdiVideoPipeline`. In zero-copy mode the pipeline sends the GPU buffer directly; when buffering is enabled it copies into a pooled ring buffer that a paced loop drains while repeating the latest frame on underruns. When an underrun occurs after the buffer has been primed, any queued frames are flushed before warm-up resumes so the most recent capture is shown before repeats. Frame-rate metadata is advertised using either the configured target cadence or the measured average.
