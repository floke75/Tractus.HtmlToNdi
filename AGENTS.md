...
* Video path: `ChromiumWebBrowser.Paint` forwards frames to the `NdiVideoPipeline`. In zero-copy mode the pipeline sends the GPU buffer directly; when buffering is enabled it copies into a pooled ring buffer, waits for it to fill to the requested depth, then drains it in FIFO order while repeating the latest frame on underruns until the bucket is primed again. Frame-rate metadata is advertised using either the configured target cadence or the measured average.
