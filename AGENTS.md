...
* Video path: `ChromiumWebBrowser.Paint` forwards frames to the `NdiVideoPipeline`. In zero-copy mode the pipeline sends the GPU buffer directly; when buffering is enabled it copies into a pooled ring buffer that an adaptive paced loop drains. The pacer phase-locks to the cadence of captured frames, only repeating the latest frame when nothing fresh has arrived by the scheduled presentation time. Frame-rate metadata is advertised using either the configured target cadence or the measured average.
