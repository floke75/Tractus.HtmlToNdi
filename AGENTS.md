...
* Video path: `ChromiumWebBrowser.Paint` forwards frames to the `NdiVideoPipeline`. In zero-copy mode the pipeline sends the GPU buffer directly; when buffering is enabled it copies into a pooled ring buffer and enforces a hysteresis-driven pacer that only leaves warm-up once the configured depth **and** an error integrator reach equilibrium. During underruns the sender repeats the last transmitted frame, aggressively discards stale captures, and drops surplus backlog via the integrator to keep effective latency fixed. Frame-rate metadata is advertised using either the configured target cadence or the measured average.
...
* `DropAllButLatest` trims the queue to a single freshest frame for underrun recovery, marking each discarded capture as stale. `TryDiscardOldestAsStale` exposes an explicit stale-drop path for the pacer’s latency integrator.
