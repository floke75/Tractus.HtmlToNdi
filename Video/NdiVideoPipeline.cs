...
internal sealed class NdiVideoPipeline : IDisposable
{
    ...
    private bool TrySendBufferedFrame()
    {
        ...
        var dropFloor = latencyExpansionActive ? targetDepth + 1 : targetDepth;
        while (latencyError > 1 && ringBuffer.Count > dropFloor)
        {
            ...
            if (latencyExpansionActive && ringBuffer.Count <= targetDepth)
            {
                break;
            }
        }
