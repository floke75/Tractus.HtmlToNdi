...
public class NdiVideoPipelineTests
{
    ...
    [Fact]
    public async Task BufferedModeDropsFramesToResyncLatency()
    {
        var sender = new CollectingSender();
        var options = new NdiVideoPipelineOptions
        {
        ...
        try
        {
            ...
            var trimmed = SpinWait.SpinUntil(() => pipeline.LatencyResyncDrops >= 1, TimeSpan.FromMilliseconds(1500));
            Assert.True(trimmed);

            Assert.True(pipeline.LatencyResyncDrops >= 1);
        }
        finally
        {
            pipeline.Dispose();
