...
| Chromium host | Create the off-screen browser, wire paint/audio hooks, expose scroll/click/keystroke helpers, and manage the paced `FramePump`. | `Chromium/CefWrapper.cs`, `Chromium/FramePump.cs`【F:Chromium/CefWrapper.cs†L11-L256】【F:Chromium/FramePump.cs†L60-L380】 |
...
3. **Chromium startup** – The process enters `AsyncContext.Run`, creates a `ChromiumWebBrowser` with audio enabled, configures frame-rate-related command-line switches, and instantiates `CefWrapper`. During `InitializeWrapperAsync` the browser loads the start URL, unmutes audio, attaches the paint handler, and spins up the pacing-aware `FramePump` that invalidates Chromium either periodically or on demand.【F:Program.cs†L231-L309】【F:Chromium/CefWrapper.cs†L40-L144】【F:Chromium/FramePump.cs†L60-L220】
4. **Pipeline wiring** – `CefWrapper` hands each `Paint` into `NdiVideoPipeline.HandleFrame`, `CustomAudioHandler` copies planar floats into a contiguous buffer, and the pipeline attaches the `FramePump` as its paced invalidation scheduler so capture cadence follows send demand.【F:Chromium/CefWrapper.cs†L43-L144】【F:Chromium/CustomAudioHandler.cs†L121-L166】【F:Video/NdiVideoPipeline.cs†L202-L420】
